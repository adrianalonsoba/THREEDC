...
function THREEDC (THREEDC,camera,scene,renderer,container,sceneCSS) {
   ...
   if(!sceneCSS){
	...
	function initScene () {
		THREEDC.scene = new THREE.Scene();
		THREEDC.renderer=renderer = new THREE.WebGLRenderer( {antialias:true} );
   		THREEDC.renderer.setSize(SCREEN_WIDTH, SCREEN_HEIGHT);
 		THREEDC.renderer.setClearColor( 0xd8d8d8 );
		THREEDC.container=container;	
		...
		var FAR = 20000;
		// set up camera
		THREEDC.camera = new THREE.PerspectiveCamera( VIEW_ANGLE, ASPECT, NEAR, FAR);
		// add the camera to the scene
		THREEDC.scene.add(THREEDC.camera);

		// automatically resize renderer
		THREEx.WindowResize(THREEDC.renderer, THREEDC.camera);
	...
	function render()
	{
	   renderer.render( THREEDC.scene, THREEDC.camera );
	}
